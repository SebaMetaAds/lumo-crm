...
export default function Opportunities(){
 ...
 <div className="conversation-list">{filtered.length===0?<div className="mini-empty">Todavía no hay oportunidades.</div>:filtered.map(o=><div key={o.id} className="conversation-item"><div className="conversation-copy"><div><strong>{o.name}</strong><small>{o.expected_close_date?new Date(o.expected_close_date+'T12:00:00').toLocaleDateString('es-AR'):''}</small></div><span>{o.sales_stages?.name||'Sin etapa'} · {o.source_channel||'Sin canal'}</span><p>{o.contacts?`${o.contacts.first_name} ${o.contacts.last_name||''}`:'Sin contacto'} · {o.amount?new Intl.NumberFormat('es-AR',{style:'currency',currency:o.currency||'ARS',maximumFractionDigits:0}).format(Number(o.amount)):'Monto a definir'}</p></div></div>)}</div>
