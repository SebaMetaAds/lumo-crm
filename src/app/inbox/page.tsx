...
export default function Inbox(){
 ...
 async function sendReply(e:FormEvent){
  e.preventDefault();if(!workspaceId||!selected||!reply.trim())return;setBusy(true);setError('');const body=reply.trim()
  try{
   const liveMeta=selected.channel_connections?.settings?.mode!=='test'&&['instagram','facebook'].includes(selected.channel_connections?.channel||'')
   if(liveMeta){
    const {data:{session}}=await supabase.auth.getSession();if(!session)throw new Error('Tu sesión expiró. Volvé a iniciar sesión.')
    const res=await fetch('/api/integrations/meta/send',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session.access_token}`},body:JSON.stringify({conversation_id:selected.id,body})})
    const result=await res.json();if(!res.ok)throw new Error(result.error||'No pudimos enviar el mensaje por Meta.')
    const data=result.message as Message;setReply('');setMessages(v=>[...v,data]);setConversations(v=>v.map(c=>c.id===selected.id?{...c,last_message_at:data.sent_at,status:'open'}:c))
   }else{
    const now=new Date().toISOString()
    const {data,error}=await supabase.from('messages').insert({workspace_id:workspaceId,conversation_id:selected.id,direction:'outgoing',sender_type:'user',sender_user_id:userId,body,message_type:'text',status:'sent',sent_at:now,attachments:[],metadata:{mode:selected.channel_connections?.settings?.mode||'test'}}).select('id,direction,sender_type,body,sent_at,status').single()
    if(error)throw error
    setReply('');setMessages(v=>[...v,data as Message]);await supabase.from('conversations').update({last_message_at:now,last_outgoing_at:now,status:'open'}).eq('id',selected.id);setConversations(v=>v.map(c=>c.id===selected.id?{...c,last_message_at:now,status:'open'}:c))
   }
  }catch(err:any){setError(err.message||'No pudimos enviar la respuesta')}finally{setBusy(false)}
 }
 ...
 <div className="test-banner"><strong>Inbox híbrido</strong><span>Podés seguir creando conversaciones de prueba. Cuando conectes Instagram/Facebook, las conversaciones reales también aparecen acá y las respuestas se envían por Meta.</span></div>
